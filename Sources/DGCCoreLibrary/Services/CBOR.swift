...
public struct UnwrappedCBOR {
    public let payload: SwiftCBOR.CBOR
    public let protected: SwiftCBOR.CBOR
    public let unprotected: [SwiftCBOR.CBOR: SwiftCBOR.CBOR]
    public let protectedBytes: [UInt8]
    public let payloadBytes: [UInt8]
    public let signatureBytes: [UInt8]
}

public enum CborType: UInt8 {
    case tag = 210
    case list = 132
    case cwt = 216
    case unknown
    
    public static func from(data: Data) -> CborType {
        switch data.first {
        case self.tag.rawValue:
            return tag
        case list.rawValue:
            return list
        case cwt.rawValue:
...
public class CBOR {
    public static func unwrap(data: Data) -> UnwrappedCBOR? {
        let type = CborType.from(data: data);
        let decoder = SwiftCBOR.CBORDecoder(input: data.uint)
        
        switch type {
        case .tag:
            guard let cbor = try? decoder.decodeItem(),
            ...
            return .init(payload: payload, protected: protected, unprotected: unprotectedMap, protectedBytes: protectedBytes,
                payloadBytes: payloadBytes, signatureBytes: signatureBytes)
            
        case .list:
            guard let cbor=try? SwiftCBOR.CBOR.decode(data.uint),
    ...
    public static func kid(from data: Data) -> [UInt8]? {
        ...
        guard let protected = unwrap?.protected,
    ...
    public static func hash(from cborData: Data) -> String {
        guard let data = COSE.signedPayloadBytes(from: cborData)
        else { return "" }
