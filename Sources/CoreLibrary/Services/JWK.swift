...
class JWK {
    public static func ecFrom(x numX: String, y numY: String) -> SecKey? {
        ...
        let errorString = error?.takeUnretainedValue().localizedDescription ?? NSLocalizedString("Something went wrong.", comment: "")
        error?.release()
        guard let key = keyReference else { print(errorString); return nil }
