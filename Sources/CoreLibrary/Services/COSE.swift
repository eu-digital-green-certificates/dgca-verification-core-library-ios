...
public class COSE {
    public static func verify(_ cbor:Data, with derPubKeyB64: String) -> Bool {
        ...
        guard let key = X509.pubKey(from: derPubKeyB64) else { return false }
        
        return Signature.verify(Data(_: sign),for: bytes,with: key)
    }

    public static func signedPayloadBytes(from cbor: Data) -> Data? {
        guard let unwrapped = CBOR.unwrap(data: cbor) else {return nil};

        let signedPayload: [UInt8] = SwiftCBOR.CBOR.encode(["Signature1", SwiftCBOR.CBOR.byteString(unwrapped.protectedBytes),
             SwiftCBOR.CBOR.byteString([]), SwiftCBOR.CBOR.byteString(unwrapped.payloadBytes)]
        )
