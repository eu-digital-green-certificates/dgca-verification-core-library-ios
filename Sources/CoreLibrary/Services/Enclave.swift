...
public class Enclave {
    ...
    static func generateKey(with name: String? = nil) -> (SecKey?, String?) {
      ...
      guard let access = SecAccessControlCreateWithFlags(kCFAllocatorDefault, kSecAttrAccessibleWhenUnlockedThisDeviceOnly,
          [.privateKeyUsage], &error)
      else { return (nil, error?.takeRetainedValue().localizedDescription) }
    ...
    static func loadKey(with name: String) -> SecKey? {
        ...
        guard status == errSecSuccess, case let result as SecKey? = item else { return nil }
    ...
    static func encrypt(data: Data, with key: SecKey) -> (Data?, String?) {
        guard let publicKey = SecKeyCopyPublicKey(key) else {
            return (nil, NSLocalizedString("Cannot retrieve public key.", comment: ""))
        }
        guard SecKeyIsAlgorithmSupported(publicKey, .encrypt, encryptAlg) else {
            return (nil, NSLocalizedString("Algorithm not supported.", comment: ""))
        }
        var error: Unmanaged<CFError>?
        let cipherData = SecKeyCreateEncryptedData(publicKey, encryptAlg, data as CFData, &error) as Data?
    ...
    static func syncDecrypt(data: Data, with key: SecKey) -> (Data?, String?) {
        ...
        var error: Unmanaged<CFError>?
        let clearData = SecKeyCreateDecryptedData(key, encryptAlg, data as CFData, &error) as Data?
        let err = error?.takeRetainedValue().localizedDescription
    ...
    static func verify(data: Data, signature: Data, with key: SecKey) -> (Bool, String?) {
        guard let publicKey = SecKeyCopyPublicKey(key) else {
            return (false, NSLocalizedString("Cannot retrieve public key.", comment: ""))
        }
        guard SecKeyIsAlgorithmSupported(publicKey, .verify, signAlg) else {
            return (false, NSLocalizedString("Algorithm not supported.", comment: ""))
        }
        var error: Unmanaged<CFError>?
        let isValid = SecKeyVerifySignature(publicKey, signAlg, data as CFData, signature as CFData, &error)
        let err = error?.takeRetainedValue().localizedDescription
        return (isValid, err)
    }

      public static func sign(data: Data, with key: SecKey, using algorithm: SecKeyAlgorithm? = nil,
      ...
      static func syncSign(data: Data, with key: SecKey, using algorithm: SecKeyAlgorithm? = nil) -> (Data?, String?) {
